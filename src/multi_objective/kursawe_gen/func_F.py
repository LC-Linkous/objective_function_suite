...
#   '.src/objective_funcs/multi_objective/kursawe_gen/func_F.py'
    ...
    try:
        if (isinstance(X, list)) or (isinstance(X, np.ndarray)):
            if (len(X)>1):
                ...
                #1 element array, so can't meet the mathematical definition for the above even though it works in code
