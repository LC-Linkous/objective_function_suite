...
filename = 'antennacat_function12_3_var_3_obj_pareto_coords_output.csv'
plotname = 'antennacat_function12_3_var_3_obj_plots.png'
...
x = np.linspace(LB_x, UB_x, 20)
y = np.linspace(LB_y, UB_y, 20)
z = np.linspace(LB_z, UB_z, 20)
...
ax1.set_ylabel('$x_2$')
ax1.set_zlabel('$x_3$')
ax1.set_title('Function 12: 3 Input Feasible Decision Space')
...
if len(pareto) < 1:
    ...
    objective_space = ax2.scatter(objective_x.flatten(), objective_y.flatten(), objective_z.flatten(), s=5, marker = 'o', color='c', alpha=0.8)
    #ax2.scatter(objective_x.flatten(), objective_y.flatten(), objective_z.flatten(), marker='*', color='black') 
    pareto_x = objective_x
    pareto_y = objective_y
    pareto_z = objective_z
...
ax2.set_title('Function 12: Pareto Front & Feasible Objective Space')
